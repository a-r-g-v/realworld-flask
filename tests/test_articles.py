...
class ArticlesUseCase(object):
    ...
    def get_article(self, slug, token=None):
        return self.client.get(
            url_for('api.ArticlesView:get', slug=slug),
            **make_client_kwargs(token=token))

    def post_article(self, token, article):
        return self.client.post(
            url_for('api.ArticlesView:post'), **make_client_kwargs(
                token=token, json=article))

    def update_article(self, token, slug, article):
        return self.client.put(
            url_for('api.ArticlesView:put', slug=slug), **make_client_kwargs(
                token=token, json=article))

    def delete_article(self, token, slug):
        return self.client.delete(
            url_for('api.ArticlesView:delete', slug=slug),
            **make_client_kwargs(token=token))

    def favorite_article(self, token, slug):
        return self.client.post(
            url_for('api.ArticlesView:favorite', slug=slug),
            **make_client_kwargs(token=token))
    ...
    def post_comment(self, token, slug, comment):
        return self.client.post(
            url_for('api.ArticlesView:post_comment', slug=slug),
            **make_client_kwargs(token=token, json=comment))

    def delete_comment(self, token, slug, comment_id):
        return self.client.delete(
            url_for(
                'api.ArticlesView:delete_comment',
                slug=slug,
    ...
    def get_comments(self, slug, token=None):
        return self.client.get(
            url_for('api.ArticlesView:get_comment', slug=slug),
            **make_client_kwargs(token=token))
...
class ArticlesTestCase(BaseTestCase, UserUseCase, ArticlesUseCase):
    def test_crud_article(self):
        ...
        new_article = generate_article()
        updated_article = self.update_article(user['user']['token'],
                                              article['article']['slug'],
                                              new_article).json
        assert 'article' in updated_article
        assert updated_article['article']['title'] == new_article['article'][
            'title']

        result = self.delete_article(user['user']['token'],
                                     updated_article['article']['slug'])
    ...
    def test_favorite(self):
        ...
        article = self.favorite_article(user['user']['token'],
